...
const (
	restAPIDefaultBatchSize    = 2    // Python INDEX_BATCH_SIZE
	restAPIDefaultMaxPages     = 1000 // Python _DEFAULT_MAX_PAGES
...
// Connector error types mirroring common/data_source/exceptions.py so callers
// (e.g. the connector /test endpoint) can map them to the same response codes
...
// support deleted-file pruning (the Python REST API connector has no slim
// snapshot interface, so prune tasks complete without deleting anything).
var ErrPruneUnsupported = errors.New("rest API connector does not support prune")

// Retry/backoff knobs mirroring Python's retry_builder and rate limiter. They
...
// NewRestAPIConnector parses a Python-compatible connector config and returns
// a connector. It performs schema validation and the base-URL SSRF check but
// performs no network I/O. Credentials are read from config["credentials"].
...
// the last value per key (Python parse_qs with v_list[-1]).
...
// Validate validates configuration and credential presence without network I/O.
// It mirrors Python's parse_storage_config + load_credentials for the pre-task
// check; live connectivity validation is performed by ValidateLive.
...
// request window (start <= updated_at < end), mirroring Python poll_source.
...
// OpenPrune reports prune as unsupported, mirroring the Python connector which
// has no slim snapshot interface (prune tasks complete as no-ops).
...
func (c *RestAPIConnector) fetchPageOnce(ctx context.Context, params map[string]any) (any, error) {
	...
	for hop := 0; hop <= restAPIMaxRedirects; hop++ {
		...
		if resp.StatusCode == http.StatusTooManyRequests {
			resp.Body.Close()
			return nil, &RateLimitTriedTooManyTimesError{Message: fmt.Sprintf("Exceeded '%d' retries", restAPI429MaxWaits)}
		}
...
func (c *RestAPIConnector) handleResponse(resp *http.Response) (any, error) {
	...
	case status >= 500:
		return nil, fmt.Errorf("REST API request failed with status %d", status)
	}
...
// validateRestAPIURLForSSRF mirrors _validate_url_for_ssrf: quick deny-list
// checks plus DNS resolution. Resolution failure is logged and tolerated, as
// in Python (the per-request check re-validates).
...
// restAPIJSONPathMatches implements the commonly used subset of the Python
// `jsonpath` package: $, dot keys, [n], [*], and .. recursive descent.
...
func (c *RestAPIConnector) itemToDocument(item map[string]any) (SourceDocument, error) {
	...
	if c.cfg.IDField != "" {
		if v := c.getTypedFieldValue(c.cfg.IDField, item); v != nil {
			...
			rawID = restAPIHash128("rest_api_item:" + pythonRepr(item))
		}
	} else {
		rawID = restAPIHash128("rest_api_item:" + pythonRepr(item))
	}
	...
	if len(c.cfg.ContentFields) > 0 {
		sem = pyStr(extractRestAPIField(item, c.cfg.ContentFields[0]))
	}
...
// renderRestAPITemplateSafeMap mirrors Python str.format_map with a SafeDict:
// unknown {name} placeholders render as the empty string; malformed placeholders
...
// pythonRepr renders a JSON value using Python's repr() conventions: single
// quotes, True/False/None, and Python float formatting. Map keys are sorted to
// keep the fallback document ID deterministic.
func pythonRepr(value any) string {
	...
	case map[string]any:
		...
		for _, k := range keys {
			parts = append(parts, pythonStringRepr(k)+": "+pythonRepr(typed[k]))
		}
		return "{" + strings.Join(parts, ", ") + "}"
	default:
		return fmt.Sprint(value)
...
// pythonStringRepr mirrors Python's repr() for strings: single-quoted with the
...
// pyStr mirrors Python's str() for the scalar and composite JSON values the
// connector deals with.
...
// pyFloatStr mirrors Python str()/repr() for floats.
...
func pyJSONDumpsTo(b *strings.Builder, value any) {
	...
	case float64:
		b.WriteString(pyFloatStr(typed))
	case []any:
		b.WriteByte('[')
...
// pythonISOFormat mirrors datetime.isoformat() for UTC-aware values.
...
// materialized in memory, mirroring Python's generator-based batching.
...
// NextBatch returns the next batch of source documents.
func (s *restAPISyncSession) NextBatch(ctx context.Context) (SyncBatch, error) {
	for len(s.pending) < s.batchSize {
		...
		for _, item := range items {
			...
			if err != nil {
				// Python skips items that fail conversion with a warning.
				continue
			}
