...
func TestRestAPIHash128AndPythonRepr(t *testing.T) {
	if got := pythonRepr(map[string]any{"a": "x", "b": 1}); got != "{'a': 'x', 'b': 1}" {
		t.Fatalf("repr dict=%q", got)
	}
	if got := pythonRepr([]any{"a", 1, true, nil}); got != "['a', 1, True, None]" {
		t.Fatalf("repr list=%q", got)
	}
	if got := pythonStringRepr("a'b\n"); got != `'a\'b\n'` {
		t.Fatalf("repr string=%q", got)
	}
	if got := pyStr(json.Number("1.0")); got != "1.0" {
		t.Fatalf("pyStr float=%q", got)
	}
	if got := pyStr(json.Number("3")); got != "3" {
		t.Fatalf("pyStr int=%q", got)
	}
