...
def _install_settings_import_stubs(monkeypatch):
    ...
    install_module("common.token_utils", num_tokens_from_string=lambda value: len(str(value).split()))
...
def test_tc_sql_601_use_sql_routes_gaussdb_prompt_through_retriever(monkeypatch, dialog_service, enable_gaussdb_docengine):
    ...
    assert retriever.sqls == [
        f"SELECT doc_id, docnm_kwd, chunk_data #>> '{{amount}}' AS amount FROM {table} "
        f"WHERE kb_id = '{kb_id}' AND CAST((chunk_data #>> '{{amount}}') AS DOUBLE PRECISION) > 100 LIMIT 128"
    ]
...
def test_tc_sql_601_gaussdb_injects_doc_scope_before_validator(monkeypatch, dialog_service, enable_gaussdb_docengine):
    ...
    assert retriever.sqls == [
        f"SELECT doc_id, docnm_kwd, chunk_data #>> '{{amount}}' AS amount FROM {table} "
        f"WHERE doc_id = '{doc_id}' AND kb_id = '{kb_id}' LIMIT 128"
    ]
...
def test_tc_sql_602_row_count_override_skips_llm_and_executes_scoped_count(
    ...
    assert chat.calls == []
    assert retriever.sqls[0] == (
        f"SELECT COUNT(*) AS rows FROM {table} WHERE kb_id = '{kb_id}' LIMIT 128"
    )
    assert retriever.sqls[1] == (
        f"SELECT doc_id, docnm_kwd FROM {table} WHERE kb_id = '{kb_id}' LIMIT 128"
    )
...
def test_tc_sql_701_use_sql_aggregate_fallback_fetches_sources_with_kb_scope(monkeypatch, dialog_service, enable_gaussdb_docengine):
    ...
    chat = FakeChatModel(
        [
            ...
            f"FROM {table} WHERE chunk_data #>> '{{dept}}' = 'finance' LIMIT 3"
        ]
    )
    ...
    assert source_sql.endswith("LIMIT 3")
...
def test_tc_sql_701_aggregate_source_builder_projects_multikb_scope_and_rejects_having(dialog_service):
    source_sql = dialog_service.gaussdb_text_to_sql.build_aggregate_source_sql(
        "SELECT SUM((chunk_data #>> '{amount}')::DOUBLE PRECISION) AS total "
        "FROM ragflow_tenant WHERE kb_id IN ('k1', 'k2') LIMIT 3",
        "docnm_kwd",
        include_kb_id=True,
    )

    assert source_sql == (
        "SELECT doc_id, docnm_kwd, kb_id FROM ragflow_tenant WHERE kb_id IN ('k1', 'k2') LIMIT 3"
    )
    ...
    with pytest.raises(ValueError) as exc_info:
        dialog_service.gaussdb_text_to_sql.build_aggregate_source_sql(
            "SELECT SUM((chunk_data #>> '{amount}')::DOUBLE PRECISION) AS total FROM ragflow_tenant "
            "GROUP BY kb_id HAVING SUM((chunk_data #>> '{amount}')::DOUBLE PRECISION) > 0",
            "docnm_kwd",
            include_kb_id=False,
        )
...
def test_tc_sql_801_use_sql_completes_multikb_reference_kb_id(monkeypatch, dialog_service, enable_gaussdb_docengine):
    ...
    chat = FakeChatModel(
        [
            ...
            f"FROM {table} WHERE kb_id IN ('{kb1}', '{kb2}')"
        ]
    )
    ...
    assert len(retriever.sqls) == 2
    assert retriever.sqls[1] == (
        f"SELECT doc_id, kb_id FROM {table} WHERE doc_id IN ('doc1', 'doc2', 'doc3') "
        f"AND kb_id IN ('{kb1}', '{kb2}') LIMIT 128"
    )
...
def test_tc_sql_803_multikb_lookup_failures_preserve_reference(
    ...
    assert result["reference"]["chunks"] == [
        {"doc_id": "doc1", "docnm_kwd": "finance.csv"}
    ]
    assert len(retriever.sqls) == 2
    if expects_warning:
...
def test_tc_sql_804_multikb_lookup_is_validated_and_scoped(
    ...
    assert retriever.sqls[1] == (
        f"SELECT doc_id, kb_id FROM {table} WHERE doc_id IN ('doc1') "
        f"AND kb_id IN ('{kb1}', '{kb2}') LIMIT 128"
    )
...
def test_tc_ret_1001_use_sql_validator_rejects_before_sql_retrieval(
    ...
    invalid_sql = (
        f"SELECT doc_id, docnm_kwd, chunk_data #>> '{{amount}}' AS amount "
        f"FROM {table} WHERE kb_id = '{other_kb_id}'"
    )
...
def test_tc_sql_1003_sql_timeout_falls_back_to_retrieval(
    ...
    retriever = ScriptedRetriever(
        [TimeoutError("query timeout"), TimeoutError("query timeout")]
    )
...
def test_tc_sql_1004_aggregate_answer_survives_source_lookup_failure(
    ...
    chat = FakeChatModel(
        [
            f"SELECT SUM(CAST(chunk_data #>> '{{amount}}' AS DOUBLE PRECISION)) AS total "
            f"FROM {table}"
        ]
    )
